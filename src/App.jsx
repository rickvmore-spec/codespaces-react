 ...
 * ------------------------------------------------------------------------
 * Implements requests 1–5 and tab-specific layouts:
 ...
 *
 * Per user requests:
 * - Casting tab shows Talent Search + Submissions Inbox + **Breakdowns header**
 * - Agent tab shows Projects/Breakdowns + Casting Offices (read-only)
 * - Talent tab mirrors Agent but hides contact details and direct submission
 * - NEW: Replace dummy roles with **Big Little Lies — Season 1** (Monterey Five + main males)
 *   and change hyperlink behavior so **role titles are clickable** → reveal the actors list
 *   (actor names themselves are not links). Ari/Maya are not hyperlinked anywhere.
 */
...
// ---------- Mock Data
// NOTE: Using placeholder headshots (rights‑cleared placeholder images). Agencies randomized among CAA, WME, Untitled, UTA.

function randAgency(i: number) {
  const a = ["CAA", "WME", "Untitled", "UTA"]; return a[i % a.length];
}

const TALENT_INIT: Talent[] = [
  // Keep demo local talent (not hyperlinked anywhere)
  {
    id: "t1",
    userId: "u4",
    stageName: "Ari James",
    ...
    headshotUrl: "https://placehold.co/200x280?text=Headshot",
  ...
  // Big Little Lies principals (Monterey Five + main males)
  { id: "tb1", stageName: "Reese Witherspoon", age: 40, heightInches: 63, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(0), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["lead"], skills: ["Comedy", "Drama"] },
  { id: "tb2", stageName: "Nicole Kidman",      age: 49, heightInches: 71, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(1), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["lead"], skills: ["Drama"] },
  { id: "tb3", stageName: "Shailene Woodley",   age: 25, heightInches: 68, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(2), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["lead"], skills: ["Drama"] },
  { id: "tb4", stageName: "Laura Dern",         age: 50, heightInches: 70, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(3), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["lead"], skills: ["Drama", "Comedy"] },
  { id: "tb5", stageName: "Zoë Kravitz",        age: 28, heightInches: 67, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(4), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["lead"], skills: ["Drama"] },
  { id: "tb6", stageName: "Alexander Skarsgård",age: 40, heightInches: 76, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(5), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["main male"], skills: ["Drama"] },
  { id: "tb7", stageName: "Adam Scott",         age: 43, heightInches: 70, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(6), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["main male"], skills: ["Comedy", "Drama"] },
  { id: "tb8", stageName: "James Tupper",       age: 51, heightInches: 73, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(7), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["main male"], skills: ["Drama"] },
  { id: "tb9", stageName: "Jeffrey Nordling",   age: 50, heightInches: 73, union: "SAG-AFTRA", city: "Los Angeles", primaryRep: randAgency(8), headshotUrl: "https://placehold.co/200x280?text=Headshot", tags: ["main male"], skills: ["Drama"] },
];

// Replace previous projects with a single BLL Season 1 project
const PROJECTS_INIT: Project[] = [
  {
    id: "p_bll",
    title: "Big Little Lies — Season 1",
    productionType: "TV",
    company: "(Demo)",
    city: "Monterey",
    notes: "Monterey Five + principal male roles",
    owners: ["u1", "u2"],
    roles: [
      { id: "r_madeline", title: "Madeline Mackenzie", description: "Lead, assertive, witty, protective mother.", gender: "Female", ageRange: [35, 45], union: "SAG-AFTRA", status: "Reviewing" },
      { id: "r_celeste",  title: "Celeste Wright",     description: "Lead, poised, complex, internal struggle.",   gender: "Female", ageRange: [35, 45], union: "SAG-AFTRA", status: "Reviewing" },
      { id: "r_jane",     title: "Jane Chapman",       description: "Lead, young single mother with resilience.", gender: "Female", ageRange: [22, 30], union: "SAG-AFTRA", status: "Reviewing" },
      { id: "r_renata",   title: "Renata Klein",       description: "Lead, powerhouse executive, sharp, fierce.", gender: "Female", ageRange: [40, 50], union: "SAG-AFTRA", status: "Reviewing" },
      { id: "r_bonnie",   title: "Bonnie Carlson",     description: "Lead, calm, grounded, artistic.",            gender: "Female", ageRange: [25, 35], union: "SAG-AFTRA", status: "Reviewing" },
      { id: "r_perry",    title: "Perry Wright",       description: "Principal male, charming with volatility.",  gender: "Male",   ageRange: [35, 45], union: "SAG-AFTRA", status: "Reviewing" },
      { id: "r_ed",       title: "Ed Mackenzie",       description: "Principal male, thoughtful, earnest.",       gender: "Male",   ageRange: [35, 45], union: "SAG-AFTRA", status: "Reviewing" },
      { id: "r_nathan",   title: "Nathan Carlson",     description: "Principal male, well-meaning ex-husband.",   gender: "Male",   ageRange: [35, 45], union: "SAG-AFTRA", status: "Reviewing" },
      { id: "r_gordon",   title: "Gordon Klein",       description: "Principal male, wealthy, defensive.",        gender: "Male",   ageRange: [40, 55], union: "SAG-AFTRA", status: "Reviewing" },
    ],
  },
];

// Map roles → real actors (submitted by agents) using our placeholder headshots
const SUBMISSIONS_INIT: Submission[] = [
  { id: "s_madeline", roleId: "r_madeline", projectId: "p_bll", talentId: "tb1", submittedBy: "Agent", coverNote: `${randAgency(0)} submission for Madeline.`, status: "Viewed", createdAt: new Date().toISOString() },
  { id: "s_celeste",  roleId: "r_celeste",  projectId: "p_bll", talentId: "tb2", submittedBy: "Agent", coverNote: `${randAgency(1)} submission for Celeste.`,  status: "Viewed", createdAt: new Date().toISOString() },
  { id: "s_jane",     roleId: "r_jane",     projectId: "p_bll", talentId: "tb3", submittedBy: "Agent", coverNote: `${randAgency(2)} submission for Jane.`,     status: "Viewed", createdAt: new Date().toISOString() },
  { id: "s_renata",   roleId: "r_renata",   projectId: "p_bll", talentId: "tb4", submittedBy: "Agent", coverNote: `${randAgency(3)} submission for Renata.`,   status: "Viewed", createdAt: new Date().toISOString() },
  { id: "s_bonnie",   roleId: "r_bonnie",   projectId: "p_bll", talentId: "tb5", submittedBy: "Agent", coverNote: `${randAgency(4)} submission for Bonnie.`,   status: "Viewed", createdAt: new Date().toISOString() },
  { id: "s_perry",    roleId: "r_perry",    projectId: "p_bll", talentId: "tb6", submittedBy: "Agent", coverNote: `${randAgency(5)} submission for Perry.`,    status: "Viewed", createdAt: new Date().toISOString() },
  { id: "s_ed",       roleId: "r_ed",       projectId: "p_bll", talentId: "tb7", submittedBy: "Agent", coverNote: `${randAgency(6)} submission for Ed.`,       status: "Viewed", createdAt: new Date().toISOString() },
  { id: "s_nathan",   roleId: "r_nathan",   projectId: "p_bll", talentId: "tb8", submittedBy: "Agent", coverNote: `${randAgency(7)} submission for Nathan.`,   status: "Viewed", createdAt: new Date().toISOString() },
  { id: "s_gordon",   roleId: "r_gordon",   projectId: "p_bll", talentId: "tb9", submittedBy: "Agent", coverNote: `${randAgency(8)} submission for Gordon.`,   status: "Viewed", createdAt: new Date().toISOString() },
];
...
export default function CastingMVPApp() {
  ...
  const isCasting = personaTab === "Casting";
  const isAgent = personaTab === "Agent";
  const isTalent = personaTab === "Talent";

  // Only allow full project management on Casting tab
  const allowManage = isCasting && canManageProjects(me);
  ...
  // UI: role expansion for BLL roles (role title is the link)
  const [expandedRoleId, setExpandedRoleId] = useState<string | null>(null);

  // ---------- Effects
  useEffect(() => {
    // demo: seed an audit for an existing self‑tape (none by default)
  }, []); // eslint-disable-line
  ...
  // Quick action stubs for breakdown → actor
  function addToAudition(subId: string) {
    logAudit({ id: `a_${Math.random().toString(36).slice(2)}`, submissionId: subId, at: isoNow(), actorUserId: me?.id || "system", action: { type: "TAG", label: "Added to Audition" } });
    alert("(stub) Added to audition session.");
  }
  function addToSession(subId: string) {
    logAudit({ id: `a_${Math.random().toString(36).slice(2)}`, submissionId: subId, at: isoNow(), actorUserId: me?.id || "system", action: { type: "TAG", label: "Added to Producer Session" } });
    alert("(stub) Added to producer session.");
  }

  // ---------- Shared subsections
  function ProjectsReadOnly() {
    return (
      <Section title="Projects & Breakdowns">
  ...
  function CastingOfficesList({ obfuscateContacts = false }: { obfuscateContacts?: boolean }) {
    const offices = [
      { name: "Hudson Studios Casting", city: "Los Angeles", email: obfuscateContacts ? "hidden" : "casting@hudson.example" },
      { name: "WeHo Casting Collective", city: "West Hollywood", email: obfuscateContacts ? "hidden" : "hello@wehocasting.example" },
    ];
    return (
      <Section title="Casting Offices">
        <ul className="divide-y">
          {offices.map((o) => (
            <li key={o.name} className="py-2 text-sm">
              <div className="font-medium">{o.name}</div>
              <div className="text-gray-600">{o.city} · {o.email}</div>
            </li>
          ))}
        </ul>
      </Section>
  ...
  // New: Breakdowns header (Casting only) — role title is the link that expands to list actors
  function BreakdownsHeader() {
    // Build cards with roles + their agent-submitted actors
    const cards = projects.map((p) => {
      const roles = p.roles.map((r) => {
        const agentSubs = submissions.filter((s) => s.projectId === p.id && s.roleId === r.id && s.submittedBy === "Agent");
        return { role: r, agentSubs };
      });
      return { project: p, roles };
    });

    return (
      <section className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        <header className="mb-3 flex items-center justify-between">
          <h2 className="text-xl font-extrabold tracking-tight">Breakdowns</h2>
          <span className="text-xs text-gray-500">role title opens actor list</span>
        </header>
        <div className="space-y-4">
          {cards.map(({ project, roles }) => (
            <div key={project.id} className="rounded-xl border p-3">
              <div className="mb-2 flex items-center justify-between">
                <div>
                  <h3 className="text-base font-semibold">{project.title}</h3>
                  <p className="text-xs text-gray-600">{project.productionType} · {project.city} · {project.company}</p>
                </div>
              </div>
              <ul className="space-y-3">
                {roles.map(({ role, agentSubs }) => (
                  <li key={role.id} className="rounded-lg border p-3">
                    <div className="mb-1 flex items-center justify-between">
                      <div>
                        <button className="text-left text-base font-medium text-blue-600 underline" onClick={() => setExpandedRoleId(expandedRoleId === role.id ? null : role.id)}>
                          {role.title}
                        </button>
                        <p className="text-xs text-gray-600">{role.description}</p>
                      </div>
                      <span className="rounded-full border px-2 py-1 text-xs">{role.status}</span>
                    </div>

                    {/* Expanded: list actors (names not hyperlinked; quick actions available) */}
                    {expandedRoleId === role.id && (
                      agentSubs.length === 0 ? (
                        <p className="text-xs text-gray-500">No agent submissions yet.</p>
                      ) : (
                        <ul className="space-y-2">
                          {agentSubs.map((s) => {
                            ...
                            return (
                              <li key={s.id} className="flex flex-wrap items-center justify-between gap-2">
                                <div className="flex items-center gap-2 min-w-0">
                                  <img src={t.headshotUrl} alt={t.stageName} className="h-12 w-9 rounded object-cover" />
                                  <div className="min-w-0">
                                    <div className="truncate text-sm font-semibold">{t.stageName}</div>
                                    <div className="text-xs text-gray-500">Rep: {t.primaryRep || "—"}</div>
                                  </div>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                  <button className="rounded-full border px-2 py-1 text-xs" onClick={() => addToAudition(s.id)}>+ Audition</button>
                                  <button className="rounded-full border px-2 py-1 text-xs" onClick={() => addToSession(s.id)}>+ Session</button>
                                  <button className="rounded-full border px-2 py-1 text-xs" onClick={() => openSelfTape(s)}>Self‑Tape</button>
                                  <button className="rounded-full border px-2 py-1 text-xs" onClick={() => setSelectedSubmission(s)}>Open</button>
                                </div>
                              </li>
                            );
                          })}
  ...
  return (
    <div className="mx-auto max-w-7xl p-4">
      ...
      {/* CASTING: Talent Search + Submissions Inbox + Breakdowns */}
      {isCasting && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          {/* Column 1: Talent Search */}
          ...
          {/* Column 3: Breakdowns header */}
          <BreakdownsHeader />
        </div>
      )}

      {/* AGENT: Projects / Roles / Breakdowns + Casting Offices (read-only) */}
      {isAgent && (
      ...
      {/* TALENT: Same as Agent but with contact info hidden & no direct submissions */}
      {isTalent && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <ProjectsReadOnly />
          <CastingOfficesList obfuscateContacts />
        </div>
      )}

      {/* PRODUCER: Minimal overview (reuse Projects read-only) */}
      {personaTab === "Producer" && (
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-gray-500">Rep: {isTalent ? "hidden" : (editTalent.primaryRep || "—")}</p>
              </div>
            </div>
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  {canChangeSubmission(me) && isCasting && (
                    <div className="flex flex-wrap gap-2">
                      {(["Viewed", "Requested Self-Tape", "Callback", "Pinned", "Avail Check", "Offer", "Booked", "Pass"] as SubmissionStatus[]).map((st) => (
                        <button key={st} onClick={() => updateSubmissionStatus(selectedSubmission.id, st)} className={classNames("rounded-full px-2 py-1 text-xs", selectedSubmission.status === st ? "bg-black text-white" : "border")}>
                          {st}
                        </button>
                      ))}
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    {!isTalent && <button className="rounded-lg bg-black px-3 py-1 text-white" onClick={() => sendMessage(selectedSubmission.id, "We pinned your submission for review.")}>Message</button>}
                  </div>
                </>
              );
            })()}
      ...
      <footer className="mt-6 text-center text-xs text-gray-500">
        Not affiliated with IMDb or Breakdown Services. Real names are for demo wiring only; replace with rights‑cleared content before distributing.
      </footer>
    </div>
  );
}
